      ...
      <aside className="artist-detail-drawer" role="dialog" aria-modal="true" aria-labelledby="artist-detail-title">
        ...
        <form className="artist-detail-drawer__form" onSubmit={submit}>
          ...
          <div className="artist-detail-drawer__row">
            ...
            <label>Price bracket<select value={form.price_bracket} onChange={(event) => update('price_bracket', event.target.value)}><option value="">Not set</option>{PRICE_OPTIONS.filter((item) => item !== '(Empty)').map((item) => <option key={item}>{item}</option>)}</select></label>
          </div>
          <div className="artist-detail-drawer__row">
