...
test('CMS Caddy block is private-upstream, upload-safe, and preserves the client IP', async () => {
  ...
  assert.match(block, /@editor path \/editor \/editor\/\*\s*handle @editor\s*\{[\s\S]*?root \* \/var\/www\/cuddlebuns\/current[\s\S]*?try_files \{path\} \/index\.html/)
  assert.match(block, /handle \/static\/\*\s*\{[\s\S]*?root \* \/var\/www\/cuddlebuns\/current/)
