...
test('PocketBase Uma adapter exposes importer fields so the sync publishes rarity, title, and GameTora thumbnails', async () => {
  ...
  assert.equal(tables.events.find(({ id }) => id === 'event-two').fields.gametora_id, 19)
  const model = createUmaModel(tables.scenarios, tables.events, tables.supportCards)
  const withoutAttachment = model.supportCards.find(({ id }) => id === 'support-ten')
  const withAttachment = model.supportCards.find(({ id }) => id === 'support-two')
  ...
  assert.equal(withAttachment.imageTaskKey, 'support-card:support-two:support.png', 'an attachment always wins over a GameTora id')
})
