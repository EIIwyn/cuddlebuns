...
test('NocoDB transform maps every collection, relations, private fields, and JSON arrays', async () => {
  ...
  assert.equal(support.fields.gametora_id, 30118)
  assert.equal(support.fields.rarity, 'SSR')
  assert.equal(support.fields.title, '[Run Forth!]')
  assert.equal('lock_facts' in support.fields, false, 'PocketBase owns lock_facts; never copied')
  const unlinkedSupport = records.find(({ collection, legacyId }) => collection === 'uma_support_cards' && legacyId === 10)
  assert.equal(unlinkedSupport.fields.gametora_id, 0, 'a blank NocoDB gametora_id maps to the PocketBase empty number')
  const scenario = records.find(({ collection, legacyId }) => collection === 'uma_scenarios' && legacyId === 1)
  assert.equal(scenario.fields.gametora_id, 5)
  assert.equal('lock_facts' in scenario.fields, false)
  const event = records.find(({ collection, legacyId }) => collection === 'uma_pvp_events' && legacyId === 2)
  assert.equal(event.fields.gametora_id, 19)
  assert.equal('lock_facts' in event.fields, false)
