...
test('PocketBase disposable container enforces schema, auth, relations, and file contracts', {
  ...
  try {
    ...
    await t.test('read-only PocketBase adapters restore legacy relations and download protected originals', async () => {
      ...
      const mappedCommission = gallery.commissions.find(({ id }) => id === commission.id)
      assert.deepEqual(mappedCommission.fields.Versions.map(({ id }) => id), [version1.id, version2.id])
      assert.deepEqual(mappedCommission.fields.Artists.map(({ id }) => id), [artist1.id, artist2.id])
      assert.deepEqual(await mappedCommission.fields.Image[0].read(), pngBytes)
      const uma = await loadPocketBaseUmaSource(readClient)
      const mappedSupport = uma.supportCards.find(({ id }) => id === support.id)
      assert.deepEqual(mappedSupport.fields.pvp_events.map(({ id }) => id), [event1.id, event2.id])
