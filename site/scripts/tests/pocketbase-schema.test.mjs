...
const expectedFields = {
  ...
  collections: {
    name: 'text', slug: 'text', display_order: 'number', visible: 'bool',
    collapsible: 'bool',
  },
  characters: {
    name: 'text', slug: 'text', subtitle: 'text', accent_color: 'text',
    card_thumbnail: 'file', display_order: 'number', visible: 'bool', collection: 'relation',
    social_label: 'text', social_url: 'url',
  },
  versions: {
    name: 'text', slug: 'text', reference_sheet: 'file',
    display_order: 'number', visible: 'bool', character: 'relation',
  },
  commissions: {
    name: 'text', type: 'text', image: 'file', source_url: 'url', date: 'date',
    published: 'bool', display_order: 'number', versions: 'relation', artists: 'relation',
  },
  uma_scenarios: {
    name: 'text', short_name: 'text', slug: 'text', era_start: 'date',
    era_end: 'date', display_color: 'text',
  },
  uma_pvp_events: {
    name: 'text', event_number: 'number', slug: 'text', event_type: 'text',
  ...
  uma_support_cards: {
    name: 'text', character_name: 'text', slug: 'text', image: 'file',
...
test('PocketBase migrations create the complete least-privilege CMS schema', async () => {
  ...
  for (const [name, fieldTypes] of Object.entries(expectedFields)) {
    ...
    assert.equal(collection.fields.some(({ name: fieldName }) => fieldName === 'legacy_id'), false,
      `${name} must not retain legacy_id`)
    assert.equal(collection.indexes.some((index) => /legacy_id/i.test(index)), false,
      `${name} must not retain legacy_id indexes`)
...
test('legacy_id removal migration removes fields and indexes from every content collection', async () => {
  const { app } = await applySchema()
  for (const name of Object.keys(expectedFields)) {
    const collection = app.collections.get(name)
    assert.equal(fieldsByName(collection).legacy_id, undefined, name)
    assert.equal(collection.indexes.some((index) => /legacy_id/i.test(index)), false, name)
  }
})
